...
class ListingController extends Controller
{
    ...
    public function byCategory($slug = '')
    {
      ...
      $category = ListingType::where('slug', $slug)->where('web_visible', 1)->first();
      ...
      $sort = request('sort', 'desc');
      if (strtolower($orderBy) == 'price') {
        $properties = Listing::where('web_visible', 1)->where('listing_type_id', $category->id)->orderBy('sale_price', $sort)->orderBy('rent_price', $sort);
      } else {
        $properties = Listing::where('web_visible', 1)->where('listing_type_id', $category->id)->orderBy($orderBy, $sort);
      }
